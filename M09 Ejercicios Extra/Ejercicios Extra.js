...
function numberOfCharacters(string) {
   ...
   for (let i = 0; i < string.length; i++) {
      if(!cantidadletras[string[i]]){
         cantidadletras[string[i]]=0;
      }
...
function deleteAbc(string) {
   ...
   // Tu código:
   let sinabc=string.replace(/[abc]/g,'');
   return sinabc;
}
...
function buscoInterseccion(array1, array2) {
   ...
   function comparar(item){
      if(array2.includes(item)&&(numiguales.includes(item)===false)){
         numiguales.push(item);
      };
   }
   return numiguales;
